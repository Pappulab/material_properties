...
% Rouse-Zimm theory applied to a single-chain model as described in
...
% analyzed at a time. Residue-residue interaction energies can be provided
% as an array saved as `relative_energies.mat`. Note temperature is assumed
...
% (<<1%) and average viscosity (<<1%). There are no qualitative differences. 
...
weighting_option = 'yes'; % Change to 'no' for unweighted Zimm matrix

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Read file with interaction energies if edge-weighting the Zimm matrix
if strcmp(weighting_option, 'yes')
    filename = 'relative_energies.mat'; % Specific to the A1-LCD
    data = load(filename);
    field_names = fieldnames(data);
    array_name = field_names{1};
    RelEl = data.(array_name);
elseif strcmp(weighting_option, 'no')
    RelEl = []; % Ignore if not weighting
else
    error('Invalid Zimm matrix weighting option.');
end
...
for k = 1:numfiles
    ...
    out10 = cell(SnapEq,1);
    out11 = cell(SnapEq,1);
    out12 = zeros(SnapEq,1);
    out13 = zeros(SnapEq,1);
    ...
    % Propagate error
    stdViscosity(k) = propagate_std_and_concat(out7,SnapEq);

    % Compliance
    avgCompliance(k) = calculate_mean_and_concat(out8);
    % Propagate error
    stdCompliance(k) = propagate_std_and_concat(out9,SnapEq);

    % Relaxation times
    tau_times{k} = calculate_mean_of_cell_array_and_concat_simple(out10); % Same number of modes
    % Standard deviation
    std_tau_times{k} = calculate_std_of_cell_array_and_concat_simple(out11);   

    % Crossover
    avgX0(k) = calculate_mean_and_concat(out12);
    avgY0(k) = calculate_mean_and_concat(out13);
    % Standard deviation
    stdX0(k) = calculate_std_and_concat(out12);
    stdY0(k) = calculate_std_and_concat(out13);

    % Save representative chain graph using last snapshot
...
% Relaxation times
relaxation_times = calculate_mean_over_replicates_simple(tau_times, Repl, Sites-1); % Same number of modes
% Standard deviation
std_relaxation_times = propagate_std_vector_over_replicates_simple(std_tau_times, Repl, Sites-1);

if strcmp(weighting_option, 'yes')
    filename_suffix = 'weighted.csv';
elseif strcmp(weighting_option, 'no')
    filename_suffix = 'unweighted.csv';
else
    error('Invalid Zimm matrix weighting option.');
end

filename1 = ['moduli_single_chain_' filename_suffix];
filename2 = ['times_single_chain_' filename_suffix];
...
write_rheology_data_to_csv(time, relaxation_modulus, std_relaxation_modulus, ...
    ...
    std_X0, crossover_y, std_Y0, filename1);
write_times_to_csv(relaxation_times, std_relaxation_times, filename2);

function [CondensateSitesG,ind_chain,siteType] = ...
    generate_site_graph(A,Length,Sites)
    
    % Makes graph of all sites in condensate with respect to non-bonded
    % interactions, plus chain connectivity. (This could be made a lot more
    % efficient.)
...
function [tvec,Gt,Freq,Storage,Loss,avg_Visc,std_Visc,avg_Comp,...
    ...
    % Boltzmann-edge-weighted single-chain sub-graphs.
    ...
    for ii = 1:NumChains
        ...
        if strcmp(weighting_option, 'yes')
            ...
            edge_type = sort(edge_type,2); % Sort each row along second dimension
        
            % Assign edge weights
            idx = sub2ind(size(RelEl),edge_type(:,1),edge_type(:,2)); % Index in RelEl
...
function result_std_propagated_vector = propagate_std_and_concat(data,SnapEq)
    % Propagate the standard deviation and concatenate the values
    vector = data(:);
    var_snap = (1/SnapEq)^2 * sum(reshape(vector.^2, SnapEq, []));
    result_std_propagated_vector = vertcat(sqrt(var_snap));
end
...
function result_mean_vector = calculate_mean_of_cell_array_and_concat_simple(data)
    % Calculate the mean and concatenate the vectors
    result_mean_vector = vertcat(mean(cell2mat(data(:))));
end

function result_std_vector = calculate_std_of_cell_array_and_concat_simple(data)
    % Calculate the standard deviation and concatenate the values
    result_std_vector = vertcat(std(cell2mat(data(:))));
end
...
function result = calculate_mean_over_replicates_simple(data, Repl, minLength)
    % Reshape the data
    reshaped_data = reshape(data, Repl, []);
    
    % Calculate the mean and reshape again
    result = reshape(mean(reshape(vertcat(reshaped_data{:}), Repl, [])), [], minLength);
end

function std_std_propagated = propagate_std_vector_over_replicates_simple(std_data, Repl, minLength)
    % Reshape the standard deviation data
    std_reshaped_data = reshape(std_data, Repl, []);

    % Error propagation for standard deviation
    var_result = (1/Repl)^2 * reshape(sum(reshape(vertcat(std_reshaped_data{:}).^2, Repl, [])), [], minLength);

    % Calculate the standard deviation
    std_std_propagated = sqrt(var_result);
end
...
function write_rheology_data_to_csv(time, relaxation_modulus, std_relaxation_modulus, ...
    ...
    % Write to .csv file
    writetable(cell2table(C_aug), filename1, 'WriteVariableNames', 0);
end

function write_times_to_csv(relaxation_times, std_relaxation_times, filename2)
    % Combine times data into a cell array
    newC = cat(2, num2cell(relaxation_times', 1), num2cell(std_relaxation_times', 1))';
    ...
    % Write to .csv file
    writetable(cell2table(C_aug), filename2, 'WriteVariableNames', 0);
end
